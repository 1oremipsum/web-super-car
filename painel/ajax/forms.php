<?php
    ...
    if(isset($_POST['signup']) && $_POST['signup'] == 'signup-client'){
        sleep(1.5);
        $nome = $_POST['nome'];
        $email = $_POST['email'];
        ...
        if($data['sucesso']){
            if(is_array($img))
                $img = Painel::uploadFile('uploads/clientes', $img);
            \view\Login::signUp($nome, $email, $senha, $img);
            $data['msg'] = "Cadastro de cliente realizado com sucesso!";
        }
        ...
        if($data['sucesso']){
            if(is_array($img)){
                $img = Painel::updateFile($img, 'uploads/clientes');
            }
