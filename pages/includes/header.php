			...
			<nav class="descktop">
				...
				<?php 
						if(!\view\Login::logado()){ 
							...
							if(count($url) >= 2){?>
								<a title="login" href="http://localhost/supercar/login"><i class="fa-solid fa-arrow-right-to-bracket"></i> <span>Login</span></a>
								<?php }else{ ?>
			...
			<nav class="mobile right">
				...
				<ul>
					...
					<li>
						<?php if(!\view\Login::logado()){ ?>
							<?php if($url >= 2){?>
								<a title="Login" class="login" href="http://localhost/supercar/login"><span>Login</span></a>
							<?php }else{ ?>
